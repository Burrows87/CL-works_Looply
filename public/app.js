            ...
            altro.slot.forEach(slotAltro => {
                mieiSlot.forEach(mioSlot => {
                    if (slotAltro.giorno === mioSlot.giorno && slotAltro.fascia === mioSlot.fascia) {
                        const matchKey = `${docSnap.id}_${mioSlot.giorno}_${mioSlot.fascia}`;
...
function proponiWhatsApp(nomeAmico, numeroAmico, giorno, fascia) {
    ...
    toast.innerHTML = `
        ...
        <div style="display:flex; gap:10px; justify-content:center;">
            <button id="btn-wa-ok" style="background:#25D366; color:white; border:none; padding:12px; border-radius:10px; cursor:pointer; font-weight:bold; flex:1;">SCRIVI</button>
            <button id="btn-wa-no" style="background:#f0f0f0; color:#666; border:none; padding:12px; border-radius:10px; cursor:pointer; flex:1;">DOPO</button>
        </div>
    `;
